...
const ResultTable: React.FC<ResultTableProps> = ({ results, onRetry, onToggleSelect, onRangeSelect, isProcessing, hideCompleted }) => {
  ...
  const visibleResults = useMemo(() => {
    ...
    if (hideCompleted) {
        filtered = filtered.filter(r => r.status !== 'success' && r.status !== 'skipped' && r.status !== 'warning');
    }
                ...
                return (
                    ...
                    <td className="px-3 py-3 text-center">
                        <input 
                            ...
                            disabled={isProcessing || item.status === 'success' || item.status === 'skipped' || item.status === 'warning'}
                    ...
                    <td className="px-4 py-3 text-center">
                        ...
                        {item.status === 'warning' && <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-yellow-100 text-yellow-800 whitespace-nowrap">Chưa có TT</span>}
