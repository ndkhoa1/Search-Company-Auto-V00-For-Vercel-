...
const App: React.FC = () => {
  ...
  const handleRetryItem = async (id: string, newName: string) => {
      ...
      try {
        ...
        // Validation check for Retry
        const hasTax = data.taxCode && data.taxCode.length > 5 && !data.taxCode.toLowerCase().includes('đang cập nhật');
        const hasDomain = data.domain && data.domain.includes('.') && !data.domain.toLowerCase().includes('không tìm thấy');
        const finalStatus = (hasTax || hasDomain) ? 'success' : 'warning';
        ...
        setSearchState(prev => ({
            ...
            results: prev.results.map(item => item.id === id ? { ...item, inputName: newName, status: finalStatus, data: data, updateHistory: history } : item)
        }));
      } catch (err: any) {
         setSearchState(prev => ({
            ...prev,
  ...
  const handleStartProcessing = async () => {
    ...
    for (let i = 0; i < currentResults.length; i++) {
      ...
      // --- REDUCED DELAY BACK TO 1000ms for Flash ---
      // Giảm delay xuống 1s để phù hợp với tốc độ của model Flash
      ...
      try {
        ...
        // --- VALIDATION LOGIC ---
        // Chỉ đánh dấu success nếu tìm thấy TaxCode hoặc Domain hợp lệ
        const hasTax = data.taxCode && data.taxCode.length > 5 && !data.taxCode.toLowerCase().includes('đang cập nhật');
        const hasDomain = data.domain && data.domain.includes('.') && !data.domain.toLowerCase().includes('không tìm thấy');
        
        // Nếu không có cả 2 -> Warning (Vàng)
        // Nếu có ít nhất 1 -> Success (Xanh)
        const finalStatus = (hasTax || hasDomain) ? 'success' : 'warning';
  ...
  const selectedCount = searchState.results.filter(r => r.selected && r.status !== 'success' && r.status !== 'skipped' && r.status !== 'warning').length;
